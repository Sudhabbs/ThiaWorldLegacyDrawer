...
const territory = [
  ...
  { id: '4', name: 'Territory 3' },
  { id: '3', name: 'Territory 4' },
  { id: '5', name: 'Territory 5' },
  { id: '6', name: 'Territory 6' },
]; 
const transactions = [
  { id: '1', date: '2024-09-01', amount: '$2000', description: 'Sale 1', territory: 200,franchise: 220, agent: 20,},
  { id: '2', date: '2024-09-05', amount: '$3000', description: 'Sale 2', territory: 300,franchise: 330, agent: 30,},
  { id: '3', date: '2024-09-10', amount: '$1500', description: 'Sale 3', territory: 150,franchise: 165, agent: 15,},
  { id: '4', date: '2024-09-12', amount: '$4000', description: 'Sale 4', territory: 400,franchise: 440, agent: 40,},
  { id: '5', date: '2024-09-15', amount: '$2500', description: 'Sale 5', territory: 250,franchise: 275, agent: 25,},
  { id: '6', date: '2024-09-20', amount: '$1000', description: 'Sale 6', territory: 100,franchise: 110, agent: 10,},
];
...
const FranchiseDashboardPage: React.FC = () => {
  ...
  const [currentTerritoryPage, setCurrentTerritoryPage] = useState(0);
  ...
  const [filteredTerritory, setFilteredTerritory] = useState(territory);
  ...
  const [totalFranchiseAmount, setTotalFranchiseAmount] = useState(0);
  ...
  useEffect(() => {
    const filtered = territory.filter(territory =>
      territory.name.toLowerCase().includes(searchQueryTerritory.toLowerCase())
    );
    setFilteredTerritory(filtered);
    setCurrentTerritoryPage(0);
  }, [searchQueryTerritory]);
  ...
  const handleTerritoryPageChange = (newPage) => {
    setCurrentTerritoryPage(newPage);
  };
  const handleSubmit = () => {
    if (!email) {
  ...
  useEffect(() => {
    const total = transactions.reduce((total, transaction) => total + transaction.franchise, 0);
    setTotalFranchiseAmount(total);
  }, []);
  
  return (
    <View style={styles.container}>
      <Header title="Franchise Dashboard" />
      <ScrollView>
        <Text style={styles.sectionTitle}>Franchise  QR Code</Text>
        <View style={styles.qrCodeContainer}>
          <QRCode value="FranchiseHeadID:XYZ123" size={100} />
        </View>

        <Text style={styles.sectionTitle}>Referral ID</Text>
        <Text style={styles.referralID}>Franchise Referral ID: XYZ123</Text>
        <Text style={styles.pageTitle}>Franchise Overview</Text>

        <View style={styles.dashboardRow}>
        <View style={styles.dashboardColumn}>
            <DashboardCard title="Territory" value={territory.length.toString()} />
          </View>
          <View style={styles.dashboardColumn}>
            <DashboardCard title="Agents" value={agents.length.toString()} />
          </View>
          <View style={styles.dashboardColumn}>
            <DashboardCard title="Customers" value={customers.length.toString()} />
          </View>
        </View>

        <View style={styles.dashboardRow}>
          <View style={styles.dashboardColumn}>
            <DashboardCard title="Total Commission" value={`$${totalFranchiseAmount}`} />
          </View>
          <View style={styles.dashboardColumn}>
        ...
        <View style={styles.dashboardRow}>
          <View style={styles.dashboardColumn}>
            <DashboardCard title="Total commision to Territory"  value={`$${totalTerritoryAmount}`} />
          </View>
          <View style={styles.dashboardColumn}>
        ...
        <Text style={styles.sectionTitle}>Territory List</Text>
          <TextInput
            placeholder="Search Territory"
            value={searchQueryTerritory}
            onChangeText={setSearchQueryTerritory}
          ...
          <DataTable>
            ...
            {filteredTerritory.slice(currentTerritoryPage * rowsPerPage, (currentTerritoryPage + 1) * rowsPerPage).map((territory) => (
              <DataTable.Row key={territory.id}>
                <DataTable.Cell>{territory.name}</DataTable.Cell>
              </DataTable.Row>
            ))}
          </DataTable>
          ...
          <View style={styles.pagination}>
            <Button
              title="Previous"
              onPress={() => handleTerritoryPageChange(Math.max(currentTerritoryPage - 1, 0))}
              disabled={currentTerritoryPage === 0}
            />
            <Text>{`Page ${currentTerritoryPage + 1} of ${Math.ceil(filteredAgents.length / rowsPerPage)}`}</Text>
            <Button
              title="Next"
              onPress={() => handleTerritoryPageChange(Math.min(currentTerritoryPage + 1, Math.ceil(filteredAgents.length / rowsPerPage) - 1))}
              disabled={(currentTerritoryPage + 1) * rowsPerPage >= filteredAgents.length}
            />
          </View>
